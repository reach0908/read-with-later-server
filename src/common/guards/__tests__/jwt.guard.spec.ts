...
describe('JwtAuthGuard', () => {
	...
	beforeEach(() => {
		jwtService = { verify: jest.fn(() => ({ type: TokenType.ACCESS, email: 'a', sub: '1' })) };
		authService = { validateUser: jest.fn() } as unknown as AuthService;
		guard = new JwtAuthGuard(jwtService as unknown as JwtService, authService);
		context = {
	...
	it('토큰 타입이 ACCESS가 아니면 UnauthorizedException을 던진다', async () => {
		jwtService.verify.mockImplementation(() => ({ type: TokenType.REFRESH, email: 'a', sub: '1' }));
	...
	it('validateUser가 null을 반환하면 UnauthorizedException을 던진다', async () => {
		jwtService.verify.mockImplementation(() => ({ type: TokenType.ACCESS, email: 'a', sub: '1' }));
